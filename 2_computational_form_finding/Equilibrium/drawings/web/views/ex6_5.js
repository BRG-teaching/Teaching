...
const view = makeTrussView({
  ...
  loads: LOADS,
  MPU: 1.9,
  ORG: [-32, -7],              // dropped clear of the caption card
  fdCenter: [17, -8],
  SFD: 4,
  frame: [[-28, -29], [31, 17]],
  titlePos: [[-20, -21.0], [17, 12.6], [17, 11.2]],
  nodeName: (i) => NAME[i],
  ...
  steps: [
    ...
    { t: 'The reactions are already decided — the load line', d: 'before designing anything: every load is vertical and the roller can only push vertically, so there is no horizontal force anywhere and the split follows from one moment equation. Laid end to end round the outside, the six loads and the two reactions make the load line on the right — and that load line is the same whatever roof you go on to draw',
      detail: (d) => [`resultant 100 kN at x = 7.500 m → B = 100 × 7.5/12 = ${Math.abs(d.reactions[4][1]).toFixed(2)} kN`,
                      ...
                      'eight external forces → eight outer spaces a…h, and the chain closes'],
    ...
    { t: 'One possible roof', d: 'a parallel-chord truss following the fall of the glass, with a vertical at every load and diagonals zig-zagging between them — deeper would be cheaper in force and worse for solar gain, which is the whole trade in this task',
      detail: (d) => [`${d.nm} members + ${d.nr} reactions = ${d.nm + d.nr}, and 2 × ${d.nn} joints = ${2 * d.nn} → the difference is ${d.det}, so it is exactly determinate`,
                      ...
                      'the official sheet proposes a different roof — 17 bars, stepped rather than parallel — which the task allows'] },
    { t: 'Now joint by joint — one Cremona', d: 'the same walk as tasks 2 to 4: start where only two forces are unknown, and hang each new point off the ONE figure that is already there. Note the far right — the roof cantilevers three metres past the roller, and the chords swap sign there',
      detail: () => ['every space of the roof is one point, named a…h outside and 1…10 inside',
                     'a member is the segment between the two points either side of it, parallel to it and to scale',
                     'pink is tension, navy compression, pale grey the members carrying nothing — those are zero-length segments'] },
  ],
  nodeStep: (o, k) => ({
    t: `Joint ${NAME[o.node]}`,
