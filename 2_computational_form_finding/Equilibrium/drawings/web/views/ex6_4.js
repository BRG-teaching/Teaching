...
const view = makeTrussView({
  ...
  steps: [
    ...
    { t: 'a) Global equilibrium — and the load line', d: 'moments about the pin: the tip load has the longest lever arm of all, so the roller ends up carrying twice what the pin does. Both still push up — the cantilever is not long enough to lift the far end. Laid end to end round the outside, the five external forces make the load line on the right, and where the two reactions meet on it is the sheet\'s division point i',
    ...
    { t: 'b) The zero members', d: 'six of them here, and the cantilever tip supplies two: joint B4 has only two members and no load, because the diagonal in that bay springs from B3 rather than B4',
      detail: () => ['T0-T1 and B0-T0 at the unloaded left top joint',
                     ...
                     'all six are zero-length segments in the Cremona: their two spaces coincide'] },
    { t: 'c) Now joint by joint — one Cremona', d: 'and this is where the two halves show themselves. Solve along and watch the top chord: navy compression over the first two panels, then pink tension from T2 onward — the change happens one panel BEFORE the roller, where the bending moment passes through zero. In the force diagram that reversal is the moment the top-chord segments cross the load line and continue on its other side',
      detail: (d) => [`top chord: ${[4, 5, 6, 7].map((m) => (Math.abs(d.forces[m]) < 1e-7 ? '0' : d.forces[m].toFixed(0))).join(' · ')} kN, left to right`,
                      'every space of the truss is one point, named a…e outside and 1…8 inside'],
