...
const view = makeTrussView({
  ...
  MPU: 2.7,
  ORG: [-25, -13.5],              // dropped clear of the caption card
  fdCenter: [17, -6],
  SFD: 3.2,
  frame: [[-27, -27], [30, 16]],
  titlePos: [[-22, -19.6], [17, 11.6], [17, 10.2]],
  nodeName: (i) => NAME[i],
  nodeLabelOff: (i) => (i < 4 ? [0, -2.0] : [0, 2.0]),
  labelSide: (m) => (m < 3 ? -1 : m < 6 ? 1 : m < 10 ? 1 : -1),
  reacLabelOff: (i) => (i === 0 ? [-2.6, -1.6] : [-1.4, 1.6]),
  // the roller on the wall pushes horizontally, straight along the top chord:
  // lift its arrow clear and hang it off the wall side rather than drawing it
  // back through the truss
  reacOff: (i) => (i === 4 ? [0, 2.6] : [0, 0]),
  reacFlip: (i) => i === 4,
  zeroLabelPos: [-8, -19.6],
  supportDir: () => [-1, 0],
  bowOff: 4.6,
  ...
  steps: [
    ...
    { t: 'a) Global equilibrium — and the load line', d: 'stacked supports cannot share a vertical load, so the pin takes all of it; the overturning is resisted by the horizontal PAIR, one panel height apart. Round the outside, the five external forces make the load line — a closed triangle here, because two of them are not vertical',
      detail: (d) => [`ΣV: the pin takes ${Math.abs(d.reactions[0][1]).toFixed(1)} kN · ΣM about it: ${Math.abs(d.reactions[4][0]).toFixed(1)} kN each way, ${2.5} m apart`,
    ...
    { t: 'b) The zero members', d: 'three this time. The top wall joint has only two members and no load, so its vertical is zero; and the far bottom joint has two members and no load, so both of those are zero as well',
      detail: () => ['B0-T0 is zero: at T0 the reaction is horizontal, so nothing is left to balance a vertical',
                     ...
                     'each is a ZERO-LENGTH segment in the Cremona: its two spaces coincide'] },
    { t: 'c) Now joint by joint — one Cremona', d: 'same rule as before — start where only two member forces are unknown. Each joint adds its point to the ONE reciprocal figure and reuses what its neighbours drew. Watch the top chord: pink the whole way, which is what a cantilever looks like',
      detail: () => ['every space is a point, a…e outside and 1…6 inside',
                     'a member is the segment between the two points either side of it',
                     'pink is tension, navy compression, pale grey the three doing nothing'] },
  ],
  nodeStep: (o, k) => ({
    t: `Joint ${NAME[o.node]}`,
